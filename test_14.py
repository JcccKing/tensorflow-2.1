...
from tensorflow.keras.layers import Conv2D,Activation,BatchNormalization,Dense,Dropout,Flatten,MaxPool2D
import os
import numpy as np
...
callpacks = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_save_path,
                                   save_best_only=True, save_weights_only=True)
history = model.fit(x_train, y_train, batch_size=32, epochs=5, validation_freq=1,
          validation_data=(x_test, y_test), callbacks=[callpacks])
model.summary()

file = open('./weights_VGGNet.txt', 'w')
